...
function menu($error = null) {
    // Array de opciones, expandir opciones aca
    $opciones = [
    ...
    if (!is_numeric($seleccion)) {
      menu('El valor ingresado no es un número.         '); // En caso de error, la función se llama a si misma (recursiva)
    } elseif ($seleccion-1 < 0 || $seleccion > count($opciones)) {
      menu('El número ingresado no es una opción válida.');
    } else {
      return $seleccion;
    }
  }